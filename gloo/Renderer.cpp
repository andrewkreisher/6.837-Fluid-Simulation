...
Renderer::Renderer(Application& application) : application_(application) {
  ...
  framebuffer_.AssociateTexture(*shadow_depth_tex_, GL_DEPTH_ATTACHMENT);
...
void Renderer::RenderShadow(RenderingInfo renderingInfo, const glm::mat4& view) const {
	framebuffer_.Bind();
	GL_CHECK(glViewport(0, 0, kShadowWidth, kShadowHeight));
	GL_CHECK(glDepthMask(GL_TRUE));
	GL_CHECK(glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE));
	GL_CHECK(glClear(GL_DEPTH_BUFFER_BIT));
	glm::mat4 world_to_light_ndc_matrix = kLightProjection * view;
	//std::unique_ptr <ShadowShader> shader = make_unique<ShadowShader>();
	...
	for (const auto& pr : renderingInfo) {
		...
		//const glm::mat4& LTW = node.GetTransform().GetLocalToWorldMatrix();

		shadow_shader_->SetTargetNode(node, pr.second);
		shadow_shader_->SetNDC(world_to_light_ndc_matrix);
		robj_ptr->Render();
  ...
  for (size_t light_id = 0; light_id < light_ptrs.size(); light_id++) {
    ...
    // directional light).
	  ...
	  if (light.CanCastShadow()) {
		  const glm::mat4 view = glm::inverse(light.GetNodePtr()->GetTransform().GetLocalToWorldMatrix());
		  RenderShadow(rendering_info, view);
	  }
    ...
    for (const auto& pr : rendering_info) {
	  ...
	  if (light.CanCastShadow()) {
		  glm::mat4 view = glm::inverse(light.GetNodePtr()->GetTransform().GetLocalToWorldMatrix());
		  glm::mat4 world_to_light_ndc_matrix = kLightProjection * view;
